...
shared abstract class Printer(Boolean pretty = false){
    ...
    "Prints an `Object`"
    shared default void printObject(JsonObject o){
        print("{");
        enter();
    ...
    "Prints an `Array`"
    shared default void printArray(JsonArray o){
        print("[");
        enter();
        variable Boolean once = true; 
        for(elem in o){
    ...
    "Prints a JSON value"
    shared default void printValue(JsonValue val){
        switch(val)
        case (is String){
            printString(val);
        }
...
Comparison compareKeys(String->JsonValue x, String->JsonValue y)
        => x.key<=>y.key; 
